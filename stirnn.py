...
def input_matrix_init(input_size, hidden_size):
	stdev = 0.001#np.sqrt(2.0 / (input_size + hidden_size))
...
def forward_backward(inputs, targets, initial_states):
	...
	for t in reversed(xrange(SEQ_SIZE)):
		...
		for d in reversed(xrange(DEPTH)):
			# compute derivative of the stabilizing term with respect to the state (Krueger et al. http://arxiv.org/pdf/1511.08400.pdf)
			h_norm = np.array([np.linalg.norm(h[d][t][b,:]) for b in xrange(BATCH_SIZE)]) 
			h_tm_norm = np.array([np.linalg.norm(h[d][t - 1][b,:]) for b in xrange(BATCH_SIZE)])
			h_tp_norm = np.array([np.linalg.norm(h[d][t + 1][b,:]) if t < SEQ_SIZE - 1 else 0 for b in xrange(BATCH_SIZE)])
			stabilizing = np.array([h[d][t][b,:] * (2 / h_norm[b]) * (2 * h_norm[b] - h_tm_norm[b] - h_tp_norm[b]) if h_norm[b] > 0 else np.zeros_like(h[d][t][b,:]) for b in xrange(BATCH_SIZE)]) 
			stabilizing /= SEQ_SIZE
			stabilizing *= STABILIZING_TERM_WEIGHT

			dhinput = relu_d(np.dot(x[d][t], Wxh[d]) + np.dot(h[d][t-1], Whh[d]) + bh[d]) * (dh * dropout[d][t] + dhh[d] + stabilizing)
...
np.save('stirnn_weights.npy', weights)